                ...
                <div class="panel-body">
                   ...
                   <div class="row">
                      ...
                      <label>{{ _tr('Title/Description') }}</label>
                        <input type="text" name="s" class="form-control" placeholder="Activity name or description" value="<?php if(isset($_GET['s'])) echo $_GET['s']; ?>">
                      </div>
                      </div>
                      <div class="col-md-2">
                      <div class="form-group">
                        ...
                        <?php if(isset($_GET['status']) || isset($_GET['s'])) { ?>
                          <a href="{{ route('all-mentor') }}" class="btn btn-xs btn-danger">{{ _tr('Clear') }}</a>
                        <?php } ?>
                      </div>
                      </div>
                   </div>
                   ...
                   <div class="row" style="margin-top: 10px;">
                    ...
                    <div class="tablescroll-primary">
                     <table class="table table-bordered table-striped ar-table"> 
                      ...
                      <tbody>
                        <?php
                        if(!empty($all_activity) && count($all_activity) > 0)
                        {
                          $sl=1;
                          foreach($all_activity as $k=>$v)
                          {
                            ...
                            <tr>
                              ...
                              <td>
                                <?php echo date('d-m-Y', strtotime($v->from_date)); ?><br/>
                                <?php echo date('d-m-Y', strtotime($v->to_date)); ?>
                              </td>
                              <td>
                              ...
                              <td>
                                <select class="ckb_isfront form-control" name="activity_plan" id="isfront<?php echo $v->ID; ?>" data="<?php echo $v->ID; ?>" <?php if($v->is_show_front == 1) { ?> style="border-color: green;" <?php } ?>>
                              ...
                              <td>
                                <a href="{{ route('add-activity') }}/{{ $v->ID }}" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-edit"></span></a>
                                <a href="{{ route('delete-activity') }}/{{ $v->ID }}" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo _tr('Sure To Delete This Activity ?'); ?>');"><span class="glyphicon glyphicon-remove"></span></a>
                              </td>
                            </tr>
                            <?php
                            $sl++;
                   ...
                   <?php if(!empty($all_activity) && count($all_activity) > 0) { echo $all_activity->links(); } ?>
                </div>
              </div>
