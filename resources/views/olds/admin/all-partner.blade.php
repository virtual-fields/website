            ...
            <div class="col-sm-12">
              ...
              <div class="panel panel-default">
                ...
                <div class="panel-heading">
                  <h3 class="panel-title">{{ _tr('All Partners') }}</h3>
                </div>
        
                <div class="panel-body">
                   
                    <form method="GET" action="{{ route('all-partner') }}" accept-charset="UTF-8">
                    ...
                    <button type="submit" class="btn btn-default" id="submitButton">{{ _tr('Search') }}</button>
					 <?php if(isset($_GET['status']) || isset($_GET['s'])) { ?>
                          <a href="{{ route('all-partner') }}" class="btn btn-xs btn-danger">{{ _tr('Clear') }}</a>
                        <?php } ?>
                    </div>
                    </div>
                    </div>
				   ...
				   <input type="hidden" name="grid_name" value="Partner">
				   <input name="_token" type="hidden" value="{{ csrf_token() }}">
				   </div>
                   ...
                   <table class="table table-bordered table-striped ar-table"> 
                    ...
                    <tbody>
                      <?php
                      if(!empty($all_partner) && count($all_partner) > 0)
                      {
                        $sl=1;
                        foreach($all_partner as $k=>$v)
                        {
                          ?>
                          <tr>
							...
							<td>
                                <input class="ajax_update_text form-control" type="text" data-id="<?php echo $v->ID; ?>" data-name="partners" name="order" id="order_<?php echo $v->ID; ?>" value="<?php echo $v->order; ?>">
                            </td>
                            <td><?php if($v->image_id > 0){ ?> <img src="<?php echo get_recource_url($v->image_id); ?>" style="width:60px;"> <?php }  ?></td>
                            <td>
                              <select class="ajx_status form-control" name="partners" id="status<?php echo $v->ID; ?>" data="<?php echo $v->ID; ?>" <?php if($v->status == 0) { ?> style="border-color: red;" <?php } ?>>
                            ...
                            <td>
                              <a href="{{ route('add-partner') }}/{{ $v->ID }}" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-edit"></span></a>
                              <a href="{{ route('delete-partner') }}/{{ $v->ID }}" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo _tr('Sure To Delete This Partner ?'); ?>');"><span class="glyphicon glyphicon-remove"></span></a>
                            </td>
                          </tr>
                          <?php
                          $sl++;
                   ...
                   <?php if(!empty($all_partner) && count($all_partner) > 0) { echo $all_partner->links(); } ?>
                </div>
				   </form>
			  </div>
