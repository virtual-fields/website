...
<div class="panel-body">
                <form name="frm" action="<?php if(!empty($single_data) && count($single_data) > 0) { ?> {{ route('upd-exp-cat') }} <?php } else { ?> {{ route('add-exp-cat') }} <?php } ?>" method="post">
                {{ csrf_field() }}
                    <div class="col-sm-5">
                        <div class="form-group">
                            <label>Category Name</label>
                            <input type="text" name="category_name" class="form-control" value="<?php if(!empty($single_data) && count($single_data) > 0) echo $single_data->category_name; else  ?>{{ old('category_name') }}">
                    ...
                    <div class="col-sm-2">
                        ...
                        <a href="{{ route('manage-expertise-category') }}" class="btn btn-sm btn-danger" style="margin-top: 26px;">Cancel</a>
                        <?php } else { ?>
                                        ...
                                        <tbody>
                                            ...
                                            <?php foreach($all_data as $c) { ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('manage-expertise-category') }}/{{ $c->ID }}" class="btn btn-xs btn-primary"><span class="glyphicon glyphicon-edit"></span></a>
                                                        <a href="{{ route('del-exp-cat') }}/{{ $c->ID }}" class="btn btn-xs btn-danger" onclick="return confirm('Sure To Delete This Category ?');"><span class="glyphicon glyphicon-remove"></span></a>
                                                    </td>
                                                </tr>
                                            <?php $sl++; } ?>
                                        </tbody>
