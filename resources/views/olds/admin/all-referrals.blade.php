...
@section('dashboard')
       ...
       <div class="panel panel-default">
        ...
        <h3 class="panel-title">{{ _tr('Referral Users of') }} <strong>{{ $parent_detail->full_name }} (<i><?php echo $parent_detail->email; ?></i>)</strong> 
		<?php if($parent_detail->role_id==2){ ?>
		<a style="float:right" href="{{ route('all-edcs') }}">{{ _tr('back') }}</a>
		<?php }else if($parent_detail->role_id==4){ ?>
		<a style="float:right" href="{{ route('all-affiliate-partner') }}">{{ _tr('back') }}</a>
		<?php } ?>
		</h3>
          ...
          <div class="panel-body">
                <form method="GET" action="{{ route('all-referrals',$id) }}" accept-charset="UTF-8">
                ...
                <input type="text" placeholder="{{ _tr('User Name/Email') }}" class="form-control" name="s" value="<?php if(isset($_GET['s'])){ echo $_GET['s']; } ?>">
					...
					<?php if(isset($_GET['s']) || isset($_GET['status'])) { ?>
                        <a href="{{ route('all-referrals',$id) }}" class="btn btn-xs btn-danger">{{ _tr('Clear') }}</a>
                    <?php } ?>
                    </div>
                    </div>
                    </div>
			...
			<form name="apply_action" id="apply_action" action="{{ route('bulk-delete') }}" method="POST">
			   ...
			   <input type="hidden" name="grid_name" value="Referral user">
			   <input name="_token" type="hidden" value="{{ csrf_token() }}">
			   </div>
            <table class="table table-bordered ar-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                          <select class="ajx_status form-control" name="users" id="status<?php echo $user->ID; ?>" data="<?php echo $user->ID; ?>" <?php if($user->status == 0) { ?> style="border-color: red;" <?php } ?>>
                        ...
                        <td>
                          <select class="ckb_isfront form-control" name="users" id="isfront<?php echo $user->ID; ?>" data="<?php echo $user->ID; ?>" <?php if($user->is_activated == 1) { ?> style="border-color: green;" <?php } ?>>
                    ...
                    <?php }else{ ?>
                    <tr>
                        <td width="100%" align="center" colspan="7"> {{ _tr('No Referral User Found !') }}</td>
                    </td>
                    <?php } ?>
                </tbody>
            </table>
            ...
            <?php if(!empty($users) && count($users) > 0) { echo $users->links(); } ?>
