            ...
            <div class="col-sm-12">
              ...
              <div class="panel panel-default">
                ...
                <div class="panel-heading">
                  <h3 class="panel-title">{{ _tr('All Roadmaps') }}</h3>
                </div>
        
                <div class="panel-body">
                   
                    <form method="GET" action="{{ route('all-roadmap') }}" accept-charset="UTF-8">
					...
					<?php if(isset($_GET['status']) || isset($_GET['s'])) { ?>
                          <a href="{{ route('all-roadmap') }}" class="btn btn-xs btn-danger">{{ _tr('Clear') }}</a>
                    <?php } ?>
                    </div>
                    </div>
                    </div>
                   ...
                   <table class="table table-bordered table-striped ar-table"> 
                    <thead>
                      <tr>
                        ...
                        <th style="width:100px;">{{ _tr('Is Done') }}</th>
                    ...
                    <tbody>
                      <?php
                      if(!empty($all_roadmap) && count($all_roadmap) > 0)
                      {
                        $sl=1;
                        foreach($all_roadmap as $k=>$v)
                        {
                          ?>
                          <tr>
                            ...
                            <td>
                                <input class="ajax_update_text form-control" type="text" data-id="<?php echo $v->ID; ?>" data-name="roadmap" name="order" id="order_<?php echo $v->ID; ?>" value="<?php echo $v->order; ?>">
                            </td>
                            <td><?php echo $v->publish_date; ?></td>
							<td>
                              <select class="ajx_is_done form-control" name="roadmap" id="status<?php echo $v->ID; ?>" data="<?php echo $v->ID; ?>" <?php if($v->is_done == 0) { ?> style="border-color: red;" <?php } ?>>
                                  <option value="1" <?php if($v->is_done == 1) { ?> selected="selected" <?php } ?>>yes</option>
                                  <option value="0" <?php if($v->is_done == 0) { ?> selected="selected" <?php } ?>>no</option>
                                </select>
                            </td>
							
                            <td>
                              <select class="ajx_status form-control" name="roadmap" id="status<?php echo $v->ID; ?>" data="<?php echo $v->ID; ?>" <?php if($v->status == 0) { ?> style="border-color: red;" <?php } ?>>
                            ...
                            <td>
                              <a href="{{ route('add-roadmap') }}/{{ $v->ID }}" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-edit"></span></a>
                              <a href="{{ route('delete-roadmap') }}/{{ $v->ID }}" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo _tr('Sure To Delete This Roadmap ?'); ?>');"><span class="glyphicon glyphicon-remove"></span></a>
                            </td>
                          </tr>
                          <?php
                          $sl++;
				  ...
				  <?php if(!empty($all_roadmap) && count($all_roadmap) > 0) { echo $all_roadmap->links(); } ?>
                </div>
              </div>
