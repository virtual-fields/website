            ...
            <div class="col-sm-12">
              ...
              <div class="panel panel-default">
                ...
                <div class="panel-heading">
                  <h3 class="panel-title">{{ _tr('All ICO Listing') }}</h3>
                </div>
        
                <div class="panel-body">
                   
                    <form method="GET" action="{{ route('all-ico-listing') }}" accept-charset="UTF-8">
                    ...
                    <button type="submit" class="btn btn-default" id="submitButton">{{ _tr('Search') }}</button>
					 <?php if(isset($_GET['status']) || isset($_GET['s'])) { ?>
                          <a href="{{ route('all-ico-listing') }}" class="btn btn-xs btn-danger">{{ _tr('Clear') }}</a>
                        <?php } ?>
                    </div>
                    </div>
                    </div>
				   ...
				   <input type="hidden" name="table" value="ico_listing">
				   <input type="hidden" name="grid_name" value="Ico_listing">
				   <input name="_token" type="hidden" value="{{ csrf_token() }}">
				   </div>
                   ...
                   <table class="table table-bordered table-striped ar-table"> 
                    ...
                    <tbody>
                      <?php
                      if(!empty($all_ico_listing) && count($all_ico_listing) > 0)
                      {
                        $sl=1;
                        foreach($all_ico_listing as $k=>$v)
                        {
                          ?>
                          <tr>
							...
							<td>
                                <input class="ajax_update_text form-control" type="text" data-id="<?php echo $v->ID; ?>" data-name="ico_listing" name="order" id="order_<?php echo $v->ID; ?>" value="<?php echo $v->order; ?>">
                            </td>
                            <td><?php if($v->image_id > 0){ ?> <img src="<?php echo get_recource_url($v->image_id); ?>" style="width:60px;"> <?php }  ?></td>
                            <td>
                              <select class="ajx_status form-control" name="ico_listing" id="status<?php echo $v->ID; ?>" data="<?php echo $v->ID; ?>" <?php if($v->status == 0) { ?> style="border-color: red;" <?php } ?>>
                            ...
                            <td>
                              <a href="{{ route('add-ico-listing') }}/{{ $v->ID }}" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-edit"></span></a>
                              <a href="{{ route('delete-ico-listing') }}/{{ $v->ID }}" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo _tr('Sure To Delete This ICO ?'); ?>');"><span class="glyphicon glyphicon-remove"></span></a>
                            </td>
                          </tr>
                          <?php
                          $sl++;
                   ...
                   <?php if(!empty($all_ico_listing) && count($all_ico_listing) > 0) { echo $all_ico_listing->links(); } ?>
                </div>
				   </form>
			  </div>
